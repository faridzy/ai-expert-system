                            ...
                            <div class="ibox-content">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover table-bordered table-striped">
                                        <tr>
                                            <td width="20%">Nama Diagnosa Penyakit</td>
                                            <td width="5%">:</td>
                                            <td>{{$diagnosaPenyakit->nama_diagnosa_penyakit}}</td>
                                        </tr>
                                    ...
                                    <a onclick="loadModal(this)" target="/backend/master/penyebab-penyakit/add?id_diagnosa={{$diagnosaPenyakit->id}}" class="btn btn-primary" title="Tambah Data"><i
                                    ...
                                    <table class="table table-striped table-bordered table-hover" id="table-role" >
                                        ...
                                        <tr>
                                            ...
                                            <th>Nama Gejala Klinis Penyebab</th>
                                            <th>Opsi</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach($data as $num => $item)
                                            <tr>
                                                ...
                                                <td>{{$item->getDiagnosaPenyakit->nama_diagnosa_penyakit}}</td>
                                                <td>{{$item->getGejalaKlinis->nama_gejala_klinis}}</td>
                                                <td align="center">
                                                    <a onclick="loadModal(this)" target="/backend/master/penyebab-penyakit/add?id_diagnosa={{$diagnosaPenyakit->id}}" data="id={{$item->id}}"
    ...
    <script>
        function deleteData(id) {
            ...
            modalConfirm("Konfirmasi", "Apakah anda yakin akan menghapus data?", function () {
                ajaxTransfer("/backend/master/penyebab-penyakit/delete", data, "#modal-output");
            })
        }
